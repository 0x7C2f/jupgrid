...
const displayAddress = `${walletAddress.slice(0, 4)}...${walletAddress.slice(
  -4
)}`;
...
Your Token Selection for A - Symbol: ${chalk.cyan(
    selectedTokenA
  )}, Address: ${chalk.cyan(selectedAddressA)}
Your Token Selection for B - Symbol: ${chalk.magenta(
    selectedTokenB
  )}, Address: ${chalk.magenta(selectedAddressB)}`);
  startInfinity();
}
...
async function infinityGrid() {
  ...
  currUsdTotalBalance =
    currentBalances.usdBalanceA + currentBalances.usdBalanceB; // Current total USD balance
  ...
  } (${currentBalances.usdBalanceB.toFixed(2)})

	${selectedTokenB} up ${spread}%: ${newPriceBUp.toFixed(5)}
	Amount of ${selectedTokenB} to send: ${infinitySellInput.toFixed(
    5
  )} (${infinitySellInputLamports} lamports)
	Amount of ${selectedTokenA} to receive: ${infinitySellOutput.toFixed(
    5
  )} (${infinitySellOutputLamports} lamports)`);
  ...
  console.log(`\n${selectedTokenB} down ${spread}%: ${newPriceBDown.toFixed(5)}
	...
	Amount of ${selectedTokenA} to receive: ${infinityBuyInput.toFixed(
    5
  )} (${infinityBuyInputLamports} lamports)`);
...
async function updateMainDisplay() {
  ...
  console.log(`-`);
  console.log(
    `\u{1F527} Settings: ${chalk.cyan(selectedTokenA)}/${chalk.magenta(
      selectedTokenB
...
Current Balance  : $${currUsdTotalBalance.toFixed(2)}`);
  ...
  if (profitOrLoss > 0) {
    ...
    console.log(
      `Loss : ${chalk.red(
        `-$${Math.abs(profitOrLoss).toFixed(2)} (-${Math.abs(
          percentageChange
        ).toFixed(2)}%)`
      )}`
    );
  ...
  console.log(`Market Change: ${(
    ((newPrice - startPrice) / startPrice) *
    100
  ).toFixed(2)}%
Performance Delta: ${(
    percentageChange -
...
-
Latest Snapshot Balance ${chalk.cyan(selectedTokenA)}: ${chalk.cyan(
    currBalanceA.toFixed(5)
  )} (Change: ${chalk.cyan((currBalanceA - initBalanceA).toFixed(5))})
Latest Snapshot Balance ${chalk.magenta(selectedTokenB)}: ${chalk.magenta(
    currBalanceB.toFixed(5)
  )} (Change: ${chalk.magenta((currBalanceB - initBalanceB).toFixed(5))})
-
Starting Balance A - ${chalk.cyan(selectedTokenA)}: ${chalk.cyan(
    initBalanceA.toFixed(5)
  )}
Starting Balance B - ${chalk.magenta(selectedTokenB)}: ${chalk.magenta(
    initBalanceB.toFixed(5)
  )}
-
Trades: ${counter}
-
        ...
        if (task !== "rebalance") {
          ...
          while (bundleChecks <= 30) {
            ...
            if (postBundleOrders !== preBundleOrders) {
              ...
              console.log(`Checking Orders for ${bundleChecks} of 30 seconds`);
              await delay(1000);
              bundleChecks++;
            }
          }
...
async function balanceCheck() {
  ...
  if (adjustmentA > 0) {
    console.log(
      `Need to trade ${chalk.cyan(
        adjustmentA / Math.pow(10, selectedDecimalsA)
      )} ${chalk.cyan(selectedTokenA)} to ${chalk.magenta(
        selectedTokenB
      )} to balance.`
    );
    ...
    console.log(
      `Need to trade ${chalk.magenta(
        adjustmentB / Math.pow(10, selectedDecimalsB)
      )} ${chalk.magenta(selectedTokenB)} to ${chalk.cyan(
        selectedTokenA
      )} to balance.`
    );
    const userConfirmation = await confirmTransaction();
